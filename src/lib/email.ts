    ...
    const { data, error } = await resend.emails.send({
      ...
      html: `
        <div style="font-family: 'Inter', sans-serif; max-width: 600px; margin: 0 auto; background-color: #0D0D0D; color: #E0E0E0; padding: 40px; border-radius: 12px; border: 1px solid #1A1A1A;">
          ...
          <div style="background-color: #1A1A1A; border: 1px solid #333333; border-radius: 8px; padding: 24px; text-align: center; margin: 30px 0;">
            <span style="font-family: monospace; font-size: 36px; font-weight: bold; letter-spacing: 8px; color: #F0A500;">${otp}</span>
          </div>
